...
graph2axes <- \(input, x, y1, y1name=y1, y2, y2name=y2, title=paste(y1name,'vs.',y2name)) {

	ggplot(input, aes(x=.data[[x]])) +
		geom_line(aes(y=.data[[y1]]), color='red') +
...
graph2axes(
	...
	'flights',
	title='Global new cases per million vs. flights per day'
)
```
...
for (country in focus) {
	...
	graph2axes(
		inner_join(flightsTemp, casesTemp, by='date'),
		'date',
		'new_cases_smoothed',
		'new cases',
		'flight_count',
		'flights',
		paste('New cases vs. flights in', country)
	) %>% print()
...
flights_extremes <- flights %>%
	group_by(location) %>%
	slice_min(flight_count, n = 1, with_ties = FALSE) %>%
	bind_rows(
		flights %>%
			group_by(location) %>%
			slice_max(flight_count, n = 1, with_ties = FALSE)
	) %>%
	mutate(extreme_type = if_else(flight_count == min(flight_count), 'min", "max")) %>%


every_country <- flights$location %>% unique()



flightImpact <- setNames(tibble(matrix(ncol = 5, nrow = 0)),
...
```{r flightimpactmap, message=FALSE, warning=FALSE}
...
```{r globalvaxxvsflights, message=FALSE, warning=FALSE}
vaxxGlobal <- covid %>%
	filter(location == "World") %>%
...
```{r vaxxpercountry, message=FALSE, warning=FALSE}
for (country in focus) {
...
```{r vaxflightcorrelation, message=FALSE, warning=FALSE}
...
```{r interactive, warning=FALSE, error=FALSE}
